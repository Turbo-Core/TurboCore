...
use jwt::SignWithKey;
use sea_orm::{EntityTrait, Set, DatabaseConnection, QueryFilter, ColumnTrait};
...
pub async fn get_at_and_rt(connection: &DatabaseConnection, uid: &String, key: &hmac::Hmac<sha2::Sha256>) -> (String, String, u32) {
    ...
    let rand_val: String = thread_rng().sample_iter(&Alphanumeric).take(5).map(char::from).collect();
